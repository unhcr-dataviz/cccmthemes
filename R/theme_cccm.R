#' CCCM [ggplot2] theme
#'
#' \code{theme_cccm} provides a basic \bold{CCCM} theme
#' to use in \bold{ggplot2} commands.
#'
#' @rdname theme_unhcr
#'
#' @param font_family Base font family "Inter".
#' You need to have "Inter" font installed on your device in order to use it.
...
#' Use `x` or `y` to have only the correspondent active. Default to `TRUE`.
#' @param axis_title_size Axis title font size. Default to `font_size` * 0.8.
#' @param axis_title_face Axis title font face. Default to "bold".
...
#' Put "transparent" for no background color. Default to "ecruwhite".
...
theme_cccm <- function(
    ...
    plot_margin = font_size,
    plot_background = "#F5F3E8",
    void = FALSE) {
    # colors
    dark_text <- "#2C2C2C"
    light_text <- "#6e6e6e"
    ...
    # margin
    ret <- ret + theme(plot.margin = margin(rep(plot_margin, 4)))
...
#'
#' @param family Font family, default "Inter".
#' @param face Font face, default "plain".
#' @param size font size in mm.
#' @param color Font color, default "#2C2C2C".
#'
#' @noRd
